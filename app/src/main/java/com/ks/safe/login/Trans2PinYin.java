...
/**
 * Created by Admin on 2017/7/3 0003 09:05.
 * Author: kang
...
public class Trans2PinYin {
    ...
    private static Trans2PinYin chineseSpelling;

    public synchronized static Trans2PinYin getInstance() {
        if (chineseSpelling == null) {
            chineseSpelling = new Trans2PinYin();
        }
    ...
    private int getChsAscii(String chs) {
        ...
        try {
            ...
            Log.i(TAG, "ERROR:ChineseSpelling.class-getChsAscii(String chs)" + e);
        }
        return asc;
    }
     ...
     * @return
     */
    public String getSingleSpelling(String chs) {
        try {
            if (chs.getBytes("gb2312").length == 2) {
                return convert(chs).substring(0, 1);
            }
     ...
     * @return
     */
    public String getFullSpelling(String chs) {
        StringBuilder buffer = new StringBuilder();
        for (int i = 0; i < chs.length(); i++) {
            buffer.append(getSingleSpelling(chs.substring(i, i + 1)));
        }
